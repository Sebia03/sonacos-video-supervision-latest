...
def alerts():
    ...
    try:
        alert_result   = get_imou_alerts(token=token, device_id=device_id, channel_id=channel_id,
                                          begin_time=begin_time, end_time=end_time, count=count, next_alarm_id=next_alarm_id)
        ...
        return jsonify({
            ...
            "beginTime": final_begin, "endTime": final_end, "count": len(cleaned_alarms),
            "nextAlarmId": next_id,
            "message": "Aucune alerte trouvée" if len(cleaned_alarms) == 0 else "Alertes récupérées avec succès",
            "rawData": raw_data, "alarms": cleaned_alarms,
        }), 200
